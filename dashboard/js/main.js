...
import { initOverview, refreshOverviewTaskWidgets } from './overview.js';
...
try {
  ...
  if (taskHandle && await verifyPermission(taskHandle, true)) {
    ...
    if (taskState && taskState.tasks) {
      refreshOverviewTaskWidgets({ tasks: taskState.tasks });
    }
    tasksLoaded = true;
  }
...
if (!tasksLoaded) {
  try {
    ...
    if (result) {
      ...
      refreshOverviewTaskWidgets(result.parsed);
      startTasksHttpWatching();
      tasksLoaded = true;
