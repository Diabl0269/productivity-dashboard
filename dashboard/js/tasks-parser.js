...
export function parseTaskMarkdown(content) {
  ...
  for (const line of lines) {
    ...
    if (headerMatch) {
      ...
      if (boldMatch) {
        title = boldMatch[1];
        description = boldMatch[2].replace(/^\s*[-—]\s*/, '').trim().replace(/<br>/g, '\n');
      }
...
export function toMarkdown(sections, tasks) {
  ...
  sections.forEach((section, idx) => {
    ...
    sectionTasks.forEach(t => {
      ...
      const escapedDesc = t.description ? t.description.replace(/\n/g, '<br>') : '';
      const descSuffix = escapedDesc ? ` — ${escapedDesc}` : '';
      const meta = formatMetaComment(t.created, t.updated, t.priority, t.taskId);
      md += `- ${checkbox} **${t.title}**${descSuffix}${meta}\n`;
